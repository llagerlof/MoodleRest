...
class MoodleRest
{
     ...
     * Constructor
     *
     * @param string|object $server_address The full URL of Moodle rest server script (eg: http://127.0.0.1/moodle/webservice/rest/server.php) or an instance of MoodleRestConnection
     * @param string $token The token generated by the moodle admin (will be ignored if MoodleRestConnection was used)
     * @param string $return_format Set the return format that will be used in request method (will be ignored if MoodleRestConnection was used)
     */
    public function __construct($server_address = null, $token = null, $return_format = self::RETURN_ARRAY)
    {
        if ($server_address instanceof MoodleRestConnection) {
            $this->server_address = $server_address->getServerAddress();
            $this->token = $server_address->getToken();
            $this->return_format = $server_address->getReturnFormat();
        } else {
            $this->server_address = $server_address;
            $this->token = $token;
            $this->return_format = $return_format;
        }
        if (!is_null($this->return_format) && $this->return_format <> 'json' && $this->return_format <> 'xml' && $this->return_format <> 'array') {
